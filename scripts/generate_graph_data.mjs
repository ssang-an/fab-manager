...
import {fabNames} from '../src/fab-campus.js';
...
for(let r=0;r<24;r++){
  const production=r>=16,homeFab=production?fabNames[Math.floor((r-16)/2)]:'R3';
  const group=production?4+Math.floor((r-16)/2):Math.floor(r/4), lane=production?r%2:r%4;
  const route={id:`R${String(r+1).padStart(2,'0')}`,name:`${['LOGIC','DRAM','NAND','CIS'][r%4]} / REV ${Math.floor(r/4)+1}`,group,homeFab,segments:[],codes:[]};
  for(let code=0;code<15;code++)route.codes.push(code===0&&r<3?['RQQF','RSAB','5TP'][r]:`RD${String(r+1).padStart(2,'0')}${String(code+1).padStart(2,'0')}`);route.program=r<16?'R&D':'PRODUCTION';
  route.entryStage=production?0:r%4===0?0:r%4;
  route.exitStage=production?11:r%5===0?5+(r%5):11-(r%3);
  let previous=null;
  for(let s=route.entryStage;s<=route.exitStage;s++){
    ...
    const count=production?4:4+(r+s)%4+(r%3);
    for(let k=0;k<count;k++){
      ...
      if(!production&&r%2===0&&s===4+r%3&&k>=count-2)nodes[n].supportFab=fabNames[(r/2)%4];
      edge(previous,n,r);route.segments.push([n]);previous=n;
      if(!production&&k===1&&s%3===1){
...
for(const [r,route] of routes.entries())for(const id of route.segments.flat()){nodes[id].fab=nodes[id].supportFab||route.homeFab;nodes[id].group=route.group;nodes[id].routeLane=r<16?r%4:r%2;}
for(let i=0;i<6000;i++){
  const r=i<1200?i%16:16+(i-1200)%8,route=routes[r];
  const path=route.segments.map(options=>options[int(0,options.length-1)]);
  const lot={id:`LOT-${String(i+1).padStart(5,'0')}`,code:route.codes[Math.floor((i<1200?i:i-1200)/(i<1200?16:8))%15],route:r,fab:nodes[path[0]].fab,program:route.program,path,events:[]};
  ...
  let t=int(route.program==='PRODUCTION'?5000:0,route.program==='PRODUCTION'?9950:8500);
  for(let p=0;p<path.length;p++){
    if(t>10080)break;
    if(p===path.length-1){lot.events.push([t,p,5]);break;}
    lot.events.push([t,p,0]);
    const stage=nodes[path[p]].stage;
    const congested=(stage===7&&i%7>=3)||(stage===4&&i%7<3);
    ...
    const arrival=t+(nodes[path[p]].fab!==nodes[path[p+1]]?.fab?int(60,180):int(8,24));
    if(t<=10080)lot.events.push(nodes[path[p]].fab!==nodes[path[p+1]]?.fab?[t,p,6,arrival,'SEND']:[t,p,6,arrival]);
    t=arrival;
  }
  lots.push(lot);
}
...
for(const fab of fabNames)for(const type of types)for(let n=1;n<=3;n++)equipment.push({id:`${fab}-${type}-${n}`,fab,type,role:n===3?'backup':'primary',availability:n===2&&type==='ETCH'?'PM':'READY'});
for(const node of nodes){
  ...
  node.equipment={};
  for(const fab of fabNames)node.equipment[fab]=node.kind==='exit'||fab!==node.fab?{primary:[],backup:[]}:{primary:equipment.filter(e=>e.fab===fab&&e.type===type&&e.role==='primary').map(e=>e.id),backup:equipment.filter(e=>e.fab===fab&&e.type===type&&e.role==='backup').map(e=>e.id)};
}
const data={version:3,fabs:fabNames,rdHub:'R3',synthetic:true,startTime,endTime,stages,statuses:['wait','job start','proc','hold','job end','fab out','move'],equipment,nodes,edges,routes,lots};
seedRecipes(data);
...
import {seedRecipes} from '../src/recipes.js';
