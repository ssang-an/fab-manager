...
test('seeded recovery obeys code ranges and does not change observed maintenance history',()=>{
 for(const tool of data.equipment){const r=tool.maintenance.find(r=>r.estimate);if(!r)continue;assert.deepEqual(r.estimate,recoveryEstimate(tool.id,r.code,r.start));assert.ok(r.estimate.hours>=r.estimate.rangeHours[0]&&r.estimate.hours<=r.estimate.rangeHours[1]);if(r.code==='COPM')assert.ok(r.estimate.hours<=12);assert.notEqual(equipmentStatus(tool,10080).status,'READY');assert.notEqual(equipmentStatus(tool,r.estimate.upAt-.01).status,'READY');assert.equal(equipmentStatus(tool,r.estimate.upAt).status,'READY');assert.equal(r.end,null);if(tool.id==='M10-ETCH-2')assert.ok(data.forecast.predictions.some(p=>p.events.some(e=>e[2]===1&&p.equipmentByStep[e[1]]===tool.id&&e[0]>=r.estimate.upAt)));}
});
...
test('down impacts identify assigned resident lots, and no processing remains on DOWN tools',()=>{
 ...
 assert.equal(rows.length,8);assert.equal(downImpact(data,states,10079).length,5);
