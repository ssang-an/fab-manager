...
export function createAnalyticsUI(api){
  ...
  $('.toolbar').insertAdjacentHTML('beforeend','<div class="equipment-search"><input id="equipment-search" list="equipment-options" placeholder="장비 검색 · A-ETCH-1" aria-label="장비명 검색"><datalist id="equipment-options"></datalist><button id="clear-equipment" aria-label="장비 검색 해제">×</button></div><button id="analysis-toggle" aria-expanded="false">분석 / 예측</button>');
  ...
  $('#equipment-search').oninput=e=>{eqQuery=e.target.value.trim().toUpperCase();api.refresh();};
  ...
  function selectEquipment(id){eqQuery=id;$('#equipment-search').value=id;show();api.refresh();}
  ...
  function update(){
    ...
    const eq=equipmentRanking(states),holds=holdRanking(states,minute),resident=states.filter(s=>s.event[2]!==5),waitMain=resident.filter(s=>s.lot.lot_type==='MAIN'&&s.event[2]===0),sample=resident.filter(s=>s.lot.lot_type==='SAMPLE');
    if(mode!=='process'){
